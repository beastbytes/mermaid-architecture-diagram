...
The building blocks of an architecture diagram are services, groups, junctions - collectively called nodes,
and edges that connect nodes.

A Service is an entity of some kind, e.g. a person, a database, ...

Groups are used to group together related services the connections between them. Groups can be nested.

Edges connect Services and Groups together.

Junctions provide a split - up to four-ways - between edges, allowing complex diagrams to be created.
...
.. code-block:: php
    ...
    Mermaid::create(ArchitectureDiagram::class)
        ->withNode(
